...
def _generate_examples(input_file):
  ...
  for line in input_file:
    ...
    for annotation in json_example["annotations"][:140]:
      ...
      if short_answers:
        ...
        short_answer_tokens = json_example["document_tokens"][
            short_answer["start_token"]:short_answer["end_token"]]
        features = {}
        #features["question"] = question_tokens_feature
        features["long"] = u" ".join(t["token"] for t in long_answer_tokens)
        features["short"] = u" ".join(t["token"] for t in short_answer_tokens)
        features["question"] = u" ".join(json_example["question_tokens"])
